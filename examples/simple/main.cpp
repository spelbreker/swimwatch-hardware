...
void connectWiFi() {
    const char* backup_ssid = "Kabouterdorp";          // Backup WiFi SSID
    const char* backup_password = "Welkom#1";  // Backup WiFi password
    String stored_ssid, stored_password;
    bool useBackup = false;
    
    if (!getWiFiCredentials(stored_ssid, stored_password)) {
        Serial.println("No stored credentials, using backup!");
        useBackup = true;
    }

    for (int attempt = 0; attempt < 2; attempt++) {
        const char* ssid = useBackup ? backup_ssid : stored_ssid.c_str();
        const char* password = useBackup ? backup_password : stored_password.c_str();
        ...
        if (!useBackup) {
            ...
            useBackup = true;  // Try backup credentials next
